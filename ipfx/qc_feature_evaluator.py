...
def qc_current_clamp_sweep(sweep, is_ramp, qc_criteria=None):
    ...
    if sweep["pre_vm_delta_mv"] is not None and sweep["pre_vm_delta_mv"] > qc_criteria["vm_delta_mv_max"]:
        fail_tags.append("Pre stimulus Vm delta: %.3f above threshold:%.3f" % (sweep["pre_vm_delta_mv"], qc_criteria["vm_delta_mv_max"]))

    if sweep["post_vm_delta_mv"] is not None and sweep["post_vm_delta_mv"] > qc_criteria["vm_delta_mv_max"]:
        fail_tags.append("Post stimulus Vm delta: %.3f above threshold:%.3f" % (sweep["post_vm_delta_mv"], qc_criteria["vm_delta_mv_max"]))
